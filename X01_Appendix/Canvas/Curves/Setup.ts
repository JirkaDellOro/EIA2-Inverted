...
class Setup {
    ...
    static init(): void {
        ...
        crc2 = <CanvasRenderingContext2D>c.getContext("2d");
        Setup.size(100, 100);
    ...
    private static printLinks(_event: Event): void {
        var scriptTags: HTMLCollectionOf<HTMLScriptElement> = document.getElementsByTagName("script");
        ...
        for (var i: number = 0; i < scriptTags.length; i++) {
            let source: string | null = scriptTags[i].getAttribute("src");
            if (source)
                names[i] = source.replace(".js", ".ts");
        }
